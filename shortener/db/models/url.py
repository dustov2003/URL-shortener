...
class UrlStorage(DeclarativeBase):
    ...
    dt_created = Column(
        ...
        server_default=func.now() + timedelta(hours=3),  # pylint: disable=not-callable
        nullable=False,
        doc="Date and time when string in table was created",
    )
    dt_deleted = Column(
        TIMESTAMP(timezone=False),
        server_default=func.now() + timedelta(hours=3) + timedelta(days=1),  # pylint: disable=not-callable
        nullable=False,
        doc="Date and time when string in table was created",
    )
